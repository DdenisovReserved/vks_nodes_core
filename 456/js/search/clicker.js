        ...
        if (phrase.length > 0) {
            $.ajax({
                ...
                success: function (data) {
                    data = $.parseJSON(data);
                    if (!data.ws.length && !data.nws.length) $("#search-results").append("<h5 class='alert alert-warning'>По вашему запросу ничего не найдено</h5>");
                    ...
                    $.each(data.nws, function (e, element) {
                        var element = searchRender.renderVks(element, 1);
                        $("#search-results").append(element);
                    });
                },
                complete: function () {
