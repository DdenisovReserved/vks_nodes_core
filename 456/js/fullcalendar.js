    ...
    function Calendar(element, instanceOptions) {
        ...
        function renderView(delta, viewName) {
            ...
            if (currentView) {
                ...
                if (typeof currentView != 'undefined' && currentView.name != 'month')
                    getLoad(currentView, cDate, currentView.calendar.options.pullServerLoadFrom);
...
//                .append('<div class="pull-left" ><input type="checkbox"  name="test-checkbox"></div>')
                    .append('<div class="fc-clear"/>');
            ...
            if (buttonStr) {
                $.each(buttonStr.split(' '), function (i) {
                    ...
                    if (groupChildren.length > 1) {
                        ...
                        //if (position == 'left'
                        //    && typeof calendar.options.pullServerLoadFrom !== 'indefined'
                        //    && calendar.options.pullServerLoadFrom == 1) {
                        //
                        //    sectionEl.append('<a class="btn btn-default" href="?route=VksNoSupport/index" type="button">К списку</a>');
                        //} else if (position == 'left'
                        //    && typeof calendar.options.pullServerLoadFrom !== 'indefined'
                        //    && calendar.options.pullServerLoadFrom == 0) {
                        //    sectionEl.append('<a class="btn btn-default" href="?route=Vks/index/' + currentDate + '" type="button">К списку</a>');
                        //}
    ...
    DragListener.prototype = {
        ...
        mousemove: function (ev) {
            ...
            if (!this.isDragging) { // if not already dragging...
                // then start the drag if the minimum distance getDbCriteria is met
...
// (such as highlight rows, day rows, helper rows, etc).
    ...
    $.extend(Grid.prototype, {
        ...
        bgCellHtml: function (row, col, date) {
            ...
            return '<td style="border-bottom:1px solid #ccc" class=' + classes.join(' ') + '" data-date="' + date.format() + '"></td>';
        },
...
// A cmp function for determining which segments should take visual priority
    ...
    $.extend(DayGrid.prototype, {
        ...
        buildSegLevels: function (segs) {
            ...
            // Give preference to elements with certain getDbCriteria, so they have
...
// A cmp function for determining the leftmost event
    ...
    $.extend(TimeGrid.prototype, {
        ...
        slatRowHtml: function () {
            ...
            //dateSent= "2015-08-26";
            //$.get("?route=Load/get/"+dateSent,{}, function(data) {
            //});
            ...
            while (slotTime < this.maxTime) {
                ...
                //take
                axisHtml = '';
                if (this.view.load_getted) {
                    var load = this.view.load.shift();
                    if (typeof load != 'undefined' && load.counter) {
                        //define color
                        var percClass = null;
                        if (load.percentage <= 25) {
                            percClass = 'load-low';
                        } else if (load.percentage >= 25 && load.percentage <= 50) {
                            percClass = 'load-mid';
                        } else if (50 <= load.percentage && load.percentage <= 75) {
                            percClass = 'load-high';
                        } else if (75 <= load.percentage && load.percentage <= 100) {
                            percClass = 'load-critical';
                        } else {
                            percClass = null;
                        }

                        axisHtml += '<td class="load-cell ' + view.widgetContentClass + '" align="right"><div style="width:' + load.percentage + '%;" class="load-plank ' + percClass + '">&nbsp</div></td><td class="load-digits-cell">' + load.counter + '/' + load.percentage + '</td>';
                    }
                    else {
                        axisHtml += '<td class="load-cell ' + view.widgetContentClass + '" align="right"><div class="load-plank-null">&nbsp</div></td><td class="load-digits-cell">0/0</td>';
                    }
        ...
        renderEventTable: function (events) {
            ...
            for (col = 0; col < segCols.length; col++) { // iterate each column grouping
                ...
                if (this.view.load_getted) {
                    ...
                    containerEl = $('<div class="fc-event-container" style="margin: 0 2.5% 0 2px;"/>');
                }
...
//        var t = "&nbsp<a href='?r=views/misc/auto&date="+date+"'><span class='glyphicon glyphicon-list text-muted'></span></a>&nbsp";
            ...
            t += date.date();
            return '' +
                '<td style="border-bottom:1px solid #e5e5e5" class="' + classes.join(' ') + '" data-date="' + date.format() + '">' +
                t +
                '</td>';
        },
...
// Responsible for managing width/height.
    ...
    function getLoad(filledElement, date, serverNum) {

        serverNum = typeof serverNum === 'undefined' ? 0 : serverNum;

        filledElement.phpDate = date.getFullYear() + "-" + addNull(date.getMonth() + 1) + "-" + date.getDate();
        ...
        $.ajax({
            ...
            type: 'GET',
            cache: false,
            url: "?route=Load/get/" + filledElement.phpDate + "/" + serverNum,
        ...
        filledElement.load = load;
        filledElement.load_getted = (filledElement.load.length != 0) ? true : false;
